...
namespace America_Virtual.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.Provincia = db.Provincias.OrderBy(e => e.Descripcion).ToList();
            ViewBag.Localidad = db.Localidades.OrderBy(e => e.Descripcion).Where(x => x.CODE == "1").ToList();
        }
        public ActionResult GetLocalidad(Guid IdProvincia)
        {
            var Localidad = db.Localidades.OrderBy(a => a.Descripcion)
            .Where(x => x.Partidos.IdProvincia == IdProvincia).Select(a => "<option value='" + a.Descripcion + "'>" + a.Descripcion + "</option>'");
